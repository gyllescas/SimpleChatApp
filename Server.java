	...
	public void run() {
		...
		try {
			...
			output_stream = new PrintStream(this.userSocket.getOutputStream());
			 String initialMessage = input_stream.readLine();
			
			
				if(initialMessage !=null){
					if(initialMessage.contains("#join")){
						this.userName = initialMessage.substring(5, initialMessage.length());
					}
				}
				...
				for (int i = 0; i < maxUsersCount; i++) {
					if (this.threads[i] != this) {
						if(this.threads[i]!=null){
							this.threads[i].output_stream.println("#newuser" + this.userName);
						}
					}
					if(this.threads[i] == this){
						this.threads[i].output_stream.println("#welcome");
					}
				}
			}
			...
			while (true) {
				String text = input_stream.readLine();
				if(text!=null){
					text = text.trim();
					if(text.compareTo("#Bye")==0){
						...
						synchronized (userThread.class) {
							for (int i = 0; i < maxUsersCount; i++) {
								...
								if(threads[i]==this){
									threads[i].output_stream.println("#Bye");
								}
							}
						}
						break;
					...
					if(text.contains("#status")){
						String sendAll = "#newStatus" + userName + " "+ text.substring(7, text.length());  
						
						synchronized (userThread.class) {
							for (int i = 0; i < maxUsersCount; i++) {
								if (threads[i] != this) {
									if(threads[i]!=null){
										threads[i].output_stream.println(sendAll);
									}
								}
								if(threads[i]==this){
									threads[i].output_stream.println("#statusPosted");
								}
							}
						}
